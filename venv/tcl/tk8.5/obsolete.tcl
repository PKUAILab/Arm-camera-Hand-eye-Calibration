...
proc ::tk::classic::restore_button {args} {
    variable prio
    if {[tk windowingsystem] eq "x11"} {
	foreach self {Button Radiobutton Checkbutton} {
	    option add *$self.borderWidth 2 $prio; # 1
	}
    }
}
...
proc ::tk::classic::restore_entry {args} {
    ...
    foreach self {Entry Spinbox} {
	if {[tk windowingsystem] ne "aqua"} {
	    option add *$self.borderWidth	2 $prio; # 1
	}
	if {[tk windowingsystem] eq "x11"} {
	    option add *$self.background		"#d9d9d9" $prio; # "white"
	    option add *$self.selectBorderWidth	1 $prio; # 0
	}
    }
}
